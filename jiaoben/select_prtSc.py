...
OUTPUT_ROOT = Path(
    "E:/yizuo_SCI/4_方案/图/结果可视化图/消融实验/tuxiangzu"
)
...
METHODS = [
    "Complete_v10",
    "1_w_o_text_intent",
    "2_w_o_frequency_branch",
    "3_w_o_spatial_branch",
    "4_w_o_Intdeg",
    "5_w_o_Intfus",
    "6_w_o_FSRC_Add",
    "1_Tardal",
...
ORIGINAL_DATASET_ROOTS = {
    "MSRS": Path("E:/yizuo_SCI/2_Datasets/MSRS/MSRS-main_autodl/test"),
}
...
def parse_args():
    ...
    parser.add_argument("--image-id", default=IMAGE_ID, help="Source image stem, for example 00427D.")
...
def method_root(method):
    ...
    if method in ABLATION_METHODS:
        return BASE_DIR / "Ours" / "ablation" / "2_images" / "MSRS" / method
...
def source_path(method, dataset, image_id):
    if dataset != "MSRS":
        raise ValueError("This comparison script is configured for MSRS only")

    if method == "Complete_v10":
        return first_existing(method_root(method) / "MSRS_full_model", image_id, [".png", ".jpg", ".jpeg", ".bmp"])

    if method in ABLATION_METHODS:
        return first_existing(method_root(method) / f"MSRS_{method}", image_id, [".png", ".jpg", ".jpeg", ".bmp"])

    if method == "2_DeF":
        return method_root(method) / "MSRS" / f"{image_id}_recover.png"

    if method == "3_MetaF":
        return first_existing(method_root(method) / "MSRS", image_id, [".jpg", ".png", ".jpeg", ".bmp"])

    if method == "4_Text-IF":
        return method_root(method) / "MSRS" / "text_fusion" / f"{image_id}.png"

    if method in TRADITIONAL_METHODS:
        return first_existing(method_root(method) / "MSRS", image_id, [".png", ".jpg", ".jpeg", ".bmp"])
...
def build_score_map(gray_images, target_method):
    target = gray_images[target_method]
    others = np.stack([image for method, image in gray_images.items() if method != target_method], axis=0)
    others_mean = others.mean(axis=0)

    target_diff = np.abs(target - others_mean)
    all_variance = np.stack(list(gray_images.values()), axis=0).var(axis=0)
    sharpness = gradient_magnitude(target)
    texture = np.abs(target - target.mean())
    saturation = ((target < 8) | (target > 247)).astype(np.float32)
...
def manual_crop_selection(full_paths, target_method, image_id):
    ...
    rs = RectangleSelector(
        ...
        spancoords="pixels",
        interactive=True,
        rectprops=dict(edgecolor="yellow", facecolor="none", linewidth=2.5),
    )
