...
OUTPUT_ROOT = Path("E:/yizuo_SCI/4_方案/图/结果可视化图/消融实验/tuxiangzu")
...
def source_path(method, dataset, image_id):
    if dataset != "MSRS":
        raise ValueError("This ablation comparison script is configured for MSRS only")

    if method == "Complete_v10":
        root = method_root(method) / "MSRS_full_model"
    else:
        root = method_root(method) / f"MSRS_{method}"
