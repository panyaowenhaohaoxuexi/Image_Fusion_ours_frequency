...
ORIGINAL_DATASET_ROOTS = {
    "M3FD": Path("E:/yizuo_SCI/2_Datasets/4_M3FD"),
}
...
def parse_args():
    ...
    parser.add_argument("--image-id", default=IMAGE_ID, help="Source image stem, for example 00000.")
...
def source_path(method, dataset, image_id):
    if dataset != "M3FD":
        raise ValueError("This comparison script is configured for M3FD only")

    if method == "Complete_v10":
        return first_existing(method_root(method) / "M3FD" / "color", image_id, [".png", ".jpg", ".jpeg", ".bmp"])

    if method == "3_MetaF":
        return first_existing(method_root(method) / "M3FD", image_id, [".jpg", ".png", ".jpeg", ".bmp"])

    if method == "4_Text-IF":
        return method_root(method) / "M3FD" / "text_fusion" / f"{image_id}.png"

    if method in COMPARISON_METHODS:
        return first_existing(method_root(method) / "M3FD", image_id, [".png", ".jpg", ".jpeg", ".bmp"])
...
def ensure_originals_exist(dataset, image_id):
    originals = {
        ...
        "VIS": find_original_image(dataset, image_id, "Vis"),
    }
...
def local_mean_std(gray, window=15):
    """Compute per-pixel local mean & std maps via integral images (O(1) per pixel).

    Returns ``(mean_map, std_map)``, each a float32 array the same shape as
    *gray*.  ``edge``-mode padding keeps borders aligned with the input.
    """
    h, w = gray.shape
    pad = window // 2
    padded = np.pad(gray, pad, mode="edge")
    integral_val = padded.cumsum(axis=0).cumsum(axis=1)
    integral_sq = (padded * padded).cumsum(axis=0).cumsum(axis=1)
    area = float(window * window)

    def _rect_sum(int_img, y1, x1):
        y2 = y1 + window - 1
        x2 = x1 + window - 1
        total = int_img[y2, x2]
        if y1 > 0:
            total -= int_img[y1 - 1, x2]
        if x1 > 0:
            total -= int_img[y2, x1 - 1]
        if y1 > 0 and x1 > 0:
            total += int_img[y1 - 1, x1 - 1]
        return total

    mean_map = np.empty((h, w), dtype=np.float32)
    std_map = np.empty((h, w), dtype=np.float32)
    for y in range(h):
        for x in range(w):
            m = _rect_sum(integral_val, y, x) / area
            sq_m = _rect_sum(integral_sq, y, x) / area
            mean_map[y, x] = m
            std_map[y, x] = np.sqrt(max(sq_m - m * m, 0.0))
    return mean_map, std_map.astype(np.float32)
